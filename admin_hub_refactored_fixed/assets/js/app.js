...
document.getElementById('loginForm').addEventListener('submit', function(e) {
  ...
  // Admin check
  if (pin === 'admin' && pass === 'doodles') {
    doLogin('admin', USERS['admin']);
    return;
  }

  // Employee first-time: PIN === password (redirect to create new password)
  if (user && user.needsSetup && pass === pin) {
    pendingSetupPin = pin;
    showSetupView();
    return;
  }
  ...
  // Returning employee
  if (user && !user.needsSetup && user.password === pass) {
    doLogin(pin, user);
    return;
  }

  // Unknown PIN but PIN===pass -> first time employee
  if (!user && pass === pin) {
    // Auto-register as employee needing setup
    USERS[pin] = { password: null, role: 'employee', name: 'Employee ' + pin, needsSetup: true };
    pendingSetupPin = pin;
    showSetupView();
    return;
  }
...
function doSetPass() {
  ...
  if (!np || np !== cp) { document.getElementById('perr').style.display = 'block'; return; }
  document.getElementById('perr').style.display = 'none';
  USERS[pendingSetupPin].password = np;
  USERS[pendingSetupPin].needsSetup = false;
  doLogin(pendingSetupPin, USERS[pendingSetupPin]);
  pendingSetupPin = null;
}
...
function doLogin(pin, user) {
  ...
  if (user.role === 'admin') {
    ...
    // Show employee UI
    document.getElementById('navRolePill').textContent = 'EMPLOYEE';
    document.getElementById('navRolePill').className = 'role-pill';
    document.getElementById('adminTabNav').style.display = 'none';
    document.getElementById('empTabNav').style.display = 'flex';
    document.getElementById('notifBtn').style.display = 'none';
    document.getElementById('btnSettings').style.display = 'none';
    document.getElementById('empWelcome').textContent = 'Welcome, ' + (user.name || pin) + '!';
    // Carry empKey into currentUser so portal can find their balance record
    if (user.empKey) currentUser.empKey = user.empKey;
    goTab('mybalance');
    loadEmployeePortal();
  }
}
...
function goTab(t) {
  ...
  if (t === 'mybalance') loadEmployeePortal();
  if (t === 'myrequests') renderMyRequests();
}
...
function submitRecordTimeOff() {
  ...
  if (type === 'Sick') {
    if (days > accr.sickBal) {
      if (!confirm(`⚠️ ${emp.name} only has ${accr.sickBal.toFixed(2)} sick days available. Record ${days} days anyway?`)) return;
    }
    ...
    if (days > accr.vacationBal) {
      if (!confirm(`⚠️ ${emp.name} only has ${accr.vacationBal.toFixed(2)} vacation days available. Record ${days} days anyway?`)) return;
    }
...
// ══════════════════════════════════════════
// CREATE USER ACCOUNT — saves to USERS
// ══════════════════════════════════════════
function submitCreateAccount() {
  const empKey = document.getElementById('userEmployeeSel').value;
  const pin    = document.getElementById('userPin').value.trim();

  if (!empKey) { alert('Please select an employee.'); return; }
  if (!pin || !/^\d{4,10}$/.test(pin)) { alert('PIN must be 4-10 digits.'); return; }
  if (USERS[pin]) { alert('A user with PIN ' + pin + ' already exists.'); return; }
  ...
  USERS[pin] = {
    password: null,
    role: 'employee',
    name: emp ? emp.name : pin,
    empKey,           // ← stored so loadEmployeePortal can find their record
    needsSetup: true
  };
  ...
  showToast('✅ Account created! Employee logs in with PIN as both username & password.');
}
...
function updateUserAccountsTab() {
  ...
  if (!tbody) return;
  const rows = Object.entries(USERS).map(([pin, user]) => `
    <tr>
      <td><strong>${pin}</strong></td>
      <td>${user.name || '—'}</td>
      <td><span class="badge ${user.role==='admin'?'bg-orange':user.role==='manager'?'bg-blue':'bg-teal'}">${user.role||'employee'}</span></td>
      <td><span class="badge ${user.needsSetup?'bg-yellow':'bg-green'}">${user.needsSetup?'Pending setup':'Set'}</span></td>
      <td><div style="display:flex;gap:6px">
        <button class="btn btn-sm" onclick="resetUserPassword('${pin}')">Reset</button>
        ${pin!=='admin'?`<button class="btn btn-red2 btn-sm" onclick="deleteUser('${pin}')">Delete</button>`:''}
      </div></td>
    </tr>`).join('');
  tbody.innerHTML = rows;
}

function resetUserPassword(pin) {
  if (!USERS[pin]) return;
  if (!confirm('Reset password for PIN ' + pin + '? They will be prompted to set a new one on next login.')) return;
  USERS[pin].password = null;
  USERS[pin].needsSetup = true;
  saveToStorage();
  updateUserAccountsTab();
  showToast('🔄 Password reset for ' + (USERS[pin].name||pin));
}

function deleteUser(pin) {
  if (!confirm('Delete user account for PIN ' + pin + '?')) return;
  delete USERS[pin];
  saveToStorage();
  updateUserAccountsTab();
...
function submitTimeOffRequest() {
  if (!currentUser) return;
  const type  = document.getElementById('myToType').value;
  const start = document.getElementById('myToStart').value;
  const end   = document.getElementById('myToEnd').value;
  const days  = parseFloat(document.getElementById('myToDays').value);
  const notes = document.getElementById('myToNotes').value;

  if (!start || !end || !days) { alert('Please fill in all fields.'); return; }

  const req = {
    id: Date.now(),
    empKey: currentUser.pin,
    empName: currentUser.name,
    type, start, end, days, notes,
    status: 'pending',
  ...
  // Clear form
  ['myToStart','myToEnd','myToDays','myToNotes'].forEach(id => { const el=document.getElementById(id); if(el) el.value=''; });

  showToast('✅ Request submitted! Your manager will review it.');
  renderMyRequests();
}
  ...
  if (empKey) {
    ...
    if (req.type === 'Sick') {
      if (req.days > accr.sickBal) {
        if (!confirm(`⚠️ ${emp.name} only has ${accr.sickBal.toFixed(2)} sick days. Approve anyway?`)) return;
      }
      ...
      if (req.days > accr.vacationBal) {
        if (!confirm(`⚠️ ${emp.name} only has ${accr.vacationBal.toFixed(2)} vacation days. Approve anyway?`)) return;
      }
...
function renderMyRequests() {
  const tbody = document.querySelector('#p-myrequests table tbody');
  if (!tbody || !currentUser) return;
  const mine = TIME_OFF_REQUESTS.filter(r => r.empKey === currentUser.pin || r.empName === currentUser.name);
  if (!mine.length) {
    tbody.innerHTML = '<tr><td colspan="6"><div class="empty"><div class="ei">📭</div><p>No requests submitted yet.</p></div></td></tr>';
    return;
  }
  tbody.innerHTML = mine.map(req => `
    <tr>
      <td><span class="badge bg-teal">${req.type}</span></td>
      <td>${req.days}</td>
      <td>${req.start}</td>
      <td>${req.end}</td>
      <td><span class="badge ${req.status==='approved'?'bg-green':req.status==='rejected'?'bg-red':'bg-yellow'}">${req.status}</span></td>
      <td style="font-size:.78rem;color:var(--text-light)">${new Date(req.submittedAt).toLocaleDateString()}</td>
    </tr>`).join('');
}

// ══════════════════════════════════════════
// EMPLOYEE PORTAL — load real balance
// ══════════════════════════════════════════
function loadEmployeePortal() {
  if (!currentUser || currentUser.role !== 'employee') return;

  // Find employee record by PIN or name
  const empKey = currentUser.empKey ||
    Object.keys(EMPLOYEES).find(k => EMPLOYEES[k].pin === currentUser.pin || EMPLOYEES[k].name === currentUser.name);

  if (!empKey || !EMPLOYEES[empKey]) {
    // No data yet
    ['empSickEarned','empSickTaken','empSickBal','empVacEarned','empVacTaken','empVacBal']
  ...
  document.getElementById('empSickEarned').textContent = accruals.sickEarned;
  document.getElementById('empSickTaken').textContent  = emp.sickTaken||0;
  document.getElementById('empSickBal').textContent    = accruals.sickBal;
  document.getElementById('empVacEarned').textContent  = accruals.vacationEarned;
  document.getElementById('empVacTaken').textContent   = emp.vacTaken||0;
  document.getElementById('empVacBal').textContent     = accruals.vacationBal;

  // Show vacation eligibility
  if (!accruals.vacationEligible) {
    document.getElementById('empVacBal').innerHTML =
      accruals.vacationBal + ' <span style="font-size:.7rem;color:#d97706" title="Cannot use vacation until 1 year of service">⚠ < 1yr</span>';
  }
...
// Hook Slack into existing submitTimeOffRequest
const _origSubmitTOR = submitTimeOffRequest;
submitTimeOffRequest = function() {
  _origSubmitTOR();
  if (SLACK_SETTINGS.notifOnRequest && SLACK_SETTINGS.webhookUrl) {
    const name = currentUser?.name || currentUser?.pin || 'An employee';
    const type = document.getElementById('myToType')?.value || 'time off';
    sendSlackMessage(`📅 *Time-Off Request* — ${name} requested ${type}. Log in to review.`);
  }
};
